...
These are the defaults for the `ContextPolicy` class. When using `MemoryClient.build_context()`, the `balanced` preset is used by default unless a policy or another preset is supplied.

| Parameter | Default | Description |
| :--- | :--- | :--- |
| `max_tokens` | (required) | Total token budget for the context. |
| `reserve_tokens` | `0` | Tokens to reserve (subtracted from `max_tokens`). |
| `min_score` | `0.0` | Minimum score required for inclusion. |
