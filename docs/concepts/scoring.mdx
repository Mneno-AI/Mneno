...
Starting in `v0.4.0`, Mneno's search and context building engines exclude candidates with no query relevance from deterministic results. This prevents unrelated memories from being included solely because of high importance, recency, or layer boosts.

- **Search**: Results with a `relevance` score of `0.0` are excluded by default.
- **Context**: Memories that do not match the query are excluded, ensuring the context package stays focused on the user's intent.

Optional rerankers can still re-order results if they have their own relevance logic.
...
| :--- | :--- | :--- |
| `recency_weight` | 0.15 | Weight for the recency signal. |
| `importance_weight` | 0.20 | Weight for the importance signal. |
| `relevance_weight` | 0.50 | Weight for the relevance (keyword/semantic) signal. |
| `access_weight` | 0.10 | Weight for the access frequency signal. |
| `freshness_weight` | 0.05 | Weight for the original creation age. |
