...
<Update label="June 18, 2026" description="v0.4.1">
...
<Update label="June 12, 2026" description="v0.4.0">

### Mneno CLI

- Added the local-first `mneno` CLI with workspace initialization, status inspection, memory creation, ranked search,
  context building, and recent-memory commands.
...
- Search now excludes candidates with no query relevance from deterministic results, while preserving optional
  reranker behavior.
- Context building uses the same relevance boundary so unrelated memories are not included solely because of
  recency or layer boosts.
